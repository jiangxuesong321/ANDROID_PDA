...
public class PrintLabelAdapter extends BaseAdapter {
    private LayoutInflater layoutInflater;
    private List<PrintLabel> objects;
    private Context context;
    private boolean isCancel;
    public PrintLabelAdapter(Context context, List<PrintLabel> objects){
    ...
    public PrintLabel getItem(int position) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null)
            convertView = layoutInflater.inflate(R.layout.li_print_label, null);
        if (position % 2 != 0) {
            if(position > 0){
        ...
        column2.setText(objects.get(position).getMaterialDes());
        column3.setText(String.valueOf(objects.get(position).getBatch()));
        column4.setText(String.valueOf(objects.get(position).getSalesDocument()));
