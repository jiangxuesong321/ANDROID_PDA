    ...
    public Map<String, String> updateBatchCharcValue(List<MaterialDocument> list) {
        ...
        try {
            for (int k = 0; k < list.size(); k++) {
                MaterialDocument materialDocument = list.get(k);
...
//                objectItem.put("SupplierBatch", "1111");//purchaseOrder.getSupplierBatch());
                ...
                long shelfLifeExpirationDate = 0;
                SimpleDateFormat sim = new SimpleDateFormat("yyyy-MM-dd");
                try {
                    shelfLifeExpirationDate = sim.parse(purchaseOrder.getShelfLifeExpirationDate()).getTime();
                } catch (Exception e) {
                }
                objectItem.put("ShelfLifeExpirationDate", "/Date(" + shelfLifeExpirationDate + ")/");
...
//                        PurchaseOrder poInfo = list.get(0);
                        for (int k = 0; k < mdList.size(); k++) {
                            MaterialDocument materialDocumentGet = mdList.get(k);
                            String urlSupplier = app.getOdataService().getHost() + app.getString(R.string.sap_url_supplier_update) +
                                    "(Material='" + materialDocumentGet.getMaterial() + "',BatchIdentifyingPlant='',Batch='" + materialDocumentGet.getBatch() + "')";
                            //获取etag
                            List<PurchaseOrder> poGetList = list.stream().filter(e -> e.getPurchaseOrderItem().equals(materialDocumentGet.getPurchaseOrderItem())).collect(Collectors.toList());
                            if (poGetList.size() > 0) {
                                ...
                                paramUpdate.put("Supplier", poGetList.get(0).getSupplier());
                                paramUpdate.put("BatchBySupplier", poGetList.get(0).getSupplierBatch());
                                paramUpdate.put("BatchIsMarkedForDeletion", false);
                                paramUpdate.put("MatlBatchIsInRstrcdUseStock", false);
